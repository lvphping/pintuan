 ...
 * ============================================================================
 * $Author: pangbin $
 * $Id: ads1.php 17217 2014-05-12 06:29:08Z pangbin $
*/
define('IN_HHS',true);
...
if ($_REQUEST['act']=='list')
{
	...
	$smarty->assign ('action_link',array ('text'=>$_LANG['ads_add'],'href'=>'ads1.php?act=add'));
...
elseif ($_REQUEST['act']=='add'){
	admin_priv ('ad_add');
	$ad_link=empty($_GET['ad_link'])?'':
	trim($_GET['ad_link']);
	$ad_name=empty($_GET['ad_name'])?'':
	trim($_GET['ad_name']);
	...
	$smarty->assign ('action_link',array ('href'=>'ads1.php?act=list','text'=>$_LANG['ad_list']));
...
elseif ($_REQUEST['act']=='insert'){
	...
	/* 插入数据 */
	$sql="INSERT INTO ".$hhs->table ('ad')." (ad_width,city_id,position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)    VALUES (			'$_POST[ad_width]',			'$_POST[city_id]',			'$_POST[position_id]',            '$_POST[media_type]',            '$ad_name',            '$ad_link',            '$ad_code',            '$start_time',            '$end_time',            '$_POST[link_man]',            '$_POST[link_email]',            '$_POST[link_phone]',            '0',            '1')";
	$db->query ($sql);
	/* 记录管理员操作 */
	...
	$link[0]['href']='ads1.php?act=list';
	$link[1]['text']=$_LANG['continue_add_ad'];
	$link[1]['href']='ads1.php?act=add';
...
elseif ($_REQUEST['act']=='update'){
	...
	$id=!empty($_POST['id'])?intval($_POST['id']):
	0;
	$type=!empty($_POST['media_type'])?intval($_POST['media_type']):
	0;
	// if ($_POST['media_type'] == '0')
	// {
	$ad_link=!empty($_POST['ad_link'])?trim($_POST['ad_link']):
	...
	/* 更新信息 */
	$sql="UPDATE ".$hhs->table ('ad')." SET "."position_id = '$_POST[position_id]', "."ad_name     = '$_POST[ad_name]', "."city_id     = '$_POST[city_id]', "."ad_width     = '$_POST[ad_width]', "."ad_link     = '$ad_link', ".$ad_code."start_time  = '$start_time', "."end_time    = '$end_time', "."link_man    = '$_POST[link_man]', "."link_email  = '$_POST[link_email]', "."link_phone  = '$_POST[link_phone]', "."enabled     = '$_POST[enabled]' "."WHERE ad_id = '$id'";
	$db->query ($sql);
	/* 记录管理员操作 */
	...
	// 清除模版缓存
	/* 提示信息 */
	$href[]=array ('text'=>$_LANG['back_ads_list'],'href'=>'ads1.php?act=list');
...
elseif ($_REQUEST['act']=='remove'){
	...
	$url='ads1.php?act=query&'.str_replace('act=remove','',$_SERVER['QUERY_STRING']);
...
function  get_adslist (){
	...
	0;
	$filter=array ();
	$filter['sort_by']=empty($_REQUEST['sort_by'])?'ad.ad_name':
	trim($_REQUEST['sort_by']);
	$filter['sort_order']=empty($_REQUEST['sort_order'])?'DESC':
	trim($_REQUEST['sort_order']);
	$filter['city_id']=empty($_REQUEST['sort_order'])?'':
	trim($_REQUEST['city_id']);
	$filter['position_id']=empty($_REQUEST['position_id'])?'':
	trim($_REQUEST['position_id']);
	$where='WHERE 1 ';
	if ($pid>0){
	...
	/* 获得广告数据 */
	$arr=array ();
	$sql='SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.'FROM '.$GLOBALS['hhs']->table ('ad').'AS ad '.'LEFT JOIN '.$GLOBALS['hhs']->table ('ad_position').' AS p ON p.position_id = ad.position_id '.'LEFT JOIN '.$GLOBALS['hhs']->table ('order_info')." AS o ON o.from_ad = ad.ad_id $where ".'GROUP BY ad.ad_id '.'ORDER by '.$filter['sort_by'].' '.$filter['sort_order'];
