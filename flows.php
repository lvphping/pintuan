...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    if (empty($goods->spec) and empty($goods->quick)) {
        $sql = "SELECT a.attr_id, a.attr_name, a.attr_type, " . "g.goods_attr_id, g.attr_value, g.attr_price " . 'FROM ' . $GLOBALS['hhs']->table('goods_attr') . ' AS g ' . 'LEFT JOIN ' . $GLOBALS['hhs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' . "WHERE a.attr_type != 0 AND g.goods_id = '" . $goods->goods_id . "' " . 'ORDER BY a.sort_order, g.attr_price, g.goods_attr_id';
    ...
    /* 检查购物车中是否有商品 */
    $sql = "SELECT COUNT(*) FROM " . $hhs->table('cart') . " WHERE session_id = '" . SESS_ID . "' " . "AND parent_id = 0 AND is_gift = 0 AND is_checked = 1 AND rec_type = '{$flow_type}'";
    if ($db->getOne($sql) == 0) {
    ...
    if ($db->getOne($sql) == 0) {
        // show_message($_LANG['no_goods_in_cart'], '', '', 'warning');
        $result = array('error' => 2, 'message' => $_LANG['no_goods_in_cart'], 'url' => './');
        die($json->encode($result));
    }
    /* 检查商品库存 */
    ...
    foreach ($all_cart_goods as $suppliers_id => $suppliers_goods) {
        ...
        if ($user_id > 0) {
            ...
            $usedSurplus += $order['surplus'];
            //使用了的余额
            $_POST['surplus'] -= $order['surplus'];
            //post数据还剩余的余额
            // 查询用户有多少积分
        ...
        /*if (isset($is_real_good)) {
            ...
            if (!$db->getOne($sql)) {
                ...
                $result = array('error' => 2, 'message' => '配送地址存在问题，请前往订单列表', 'url' => 'user.php?act=order_list');
                die($json->encode($result));
            }
        }*/
        ...
        if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0) {
            ...
            if ($order['order_amount'] > $user_info['user_money'] + $user_info['credit_line']) {
                $result = array('error' => 2, 'message' => $_LANG['balance_not_enough'], 'url' => 'user.php?act=order_list');
                die($json->encode($result));
        ...
        /* 插入订单商品 */
        $sql = "INSERT INTO " . $hhs->table('order_goods') . "( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,rate_1,rate_2,rate_3) " . " SELECT '{$new_order_id}', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,rate_1,rate_2,rate_3" . " FROM " . $hhs->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND is_checked=1 AND rec_type = '{$flow_type}' AND suppliers_id = '{$suppliers_id}'";
        $db->query($sql);
        /* 修改拍卖活动状态 */
    ...
    if ($order_amount > 0) {
        if ($pay_code == 'wxpay') {
            ...
            $order = array('order_amount' => $order_amount, 'order_sn' => time() . rand(99, 9999), 'goods_name' => join(',', $order_goods_name));
            $pay_obj = new $payment['pay_code']();
            $pay_online = $pay_obj->get_code2($order, unserialize_config($payment['pay_config']), $created_orders);
        }
    } else {
        //require(ROOT_PATH . 'includes/lib_order.php');
        $result = array('error' => 1, 'message' => 'user.php', 'url' => "user.php?act=order_detail&order_id=" . $new_order_id . "&uid=" . $uid);
    ...
    $result = array('error' => 0, 'message' => '', 'pay_code' => $pay_code, 'content' => $pay_online, 'order_id' => join(',', $created_orders), 'url' => 'user.php?act=order_list');
    die($json->encode($result));
    ...
    if ($rec_id && $goods_number) {
        flow_update_cart(array($rec_id => $goods_number));
    }
    ...
    $data = array('count' => $cart_goods['total']['real_goods_count'], 'amount' => $cart_goods['total']['goods_amount']);
    include_once 'includes/cls_json.php';
    $json = new JSON();
    $result = array('error' => 0, 'message' => '', 'content' => '', 'rec_id' => $rec_id, 'goods_number' => $goods_number, 'subtotal' => $subtotal, 'data' => $data);
    die($json->encode($result));
    exit;
    ...
    include_once 'includes/cls_json.php';
    $json = new JSON();
    $result = array('error' => 0, 'message' => '', 'content' => '', 'data' => $data);
    die($json->encode($result));
    exit;
    ...
    include_once 'includes/cls_json.php';
    $json = new JSON();
    $result = array('error' => 0, 'message' => '', 'content' => '', 'rec_id' => $rec_id, 'data' => $data);
    die($json->encode($result));
    exit;
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr as $key => $val) {
        ...
        $sql = "SELECT g.goods_name, g.goods_number, g.limit_buy_bumber,g.promote_start_date,g.promote_end_date " . "FROM " . $GLOBALS['hhs']->table('goods') . " AS g, " . $GLOBALS['hhs']->table('cart') . " AS c " . "WHERE g.goods_id = c.goods_id AND c.rec_id = '{$key}'";
        ...
        if ($row['limit_buy_bumber'] > 0 && $val > $row['limit_buy_bumber']) {
            $result = array('error' => 1, 'message' => '该商品限购，一个用户只能购买' . $row['limit_buy_bumber'], 'goods_number' => $row['limit_buy_bumber'], 'rec_id' => $key, 'content' => '');
            die($json->encode($result));
        }
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy') {
            if ($row['goods_number'] < $val) {
                ...
                $result = array('error' => 1, 'message' => sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'], $row['goods_number'], $row['goods_number']), 'goods_number' => $row['goods_number'], 'rec_id' => $key, 'content' => '');
                die($json->encode($result));
                exit;
            }
            /* 是货品 */
            ...
            if (!empty($goods['product_id'])) {
                ...
                if ($product_number < $val) {
                    ...
                    // exit;
                    $result = array('error' => 1, 'message' => sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'], $product_number['product_number'], $product_number['product_number']), 'goods_number' => $product_number['product_number'], 'rec_id' => $key, 'content' => '');
                    die($json->encode($result));
                }
            }
