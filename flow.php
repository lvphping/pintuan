...
assign_template();
$smarty->assign('page_title', $_LANG['shopping_flow']);
// 页面标题
...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    if (!is_numeric($goods->number) || intval($goods->number) <= 0) {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $goods->rec_type, $goods->team_sign)) {
            ...
            $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' . ' FROM ' . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    ...
    if ((!isset($_CFG['use_integral']) || $_CFG['use_integral'] == '1') && $_SESSION['user_id'] > 0 && $user_info['pay_points'] > 0 && $flow_type != CART_GROUP_BUY_GOODS) {
        // 能使用积分
    ...
    if ($cart_goods[0]['bonus_allowed'] && (!isset($_CFG['use_bonus']) || $_CFG['use_bonus'] == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
        // 取得用户可用红包
    ...
    /* 插入订单商品 */
    $sql = "INSERT INTO " . $hhs->table('order_goods') . "( " . "city_id,district_id,suppliers_id,order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,rate_1,rate_2,rate_3) " . " SELECT city_id,district_id,suppliers_id,'{$new_order_id}', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, '{$_SESSION['extension_code']}' , parent_id, is_gift, goods_attr_id,rate_1,rate_2,rate_3" . " FROM " . $hhs->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND rec_type = '{$flow_type}'";
    $db->query($sql);
    if ($order['extension_code'] == 'team_goods') {
