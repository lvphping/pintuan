...
{
    $sql = 'SELECT shipping_id, shipping_name ' . 'FROM ' . $GLOBALS['hhs']->table('shipping') . ' WHERE enabled = 1';
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['hhs']->table('shipping') . " WHERE shipping_id = '{$shipping_id}' " . 'AND enabled = 1';
...
function available_shipping_list($region_id_list, $suppliers_id = 0, $goods_id_list = array())
{
    ...
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['hhs']->table('shipping') . ' AS s, ' . $GLOBALS['hhs']->table('shipping_area') . ' AS a, ' . $GLOBALS['hhs']->table('area_region') . ' AS r ' . 'WHERE r.region_id ' . db_create_in($region_id_list) . $andwhere . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 ORDER BY s.shipping_order';
    return $GLOBALS['db']->getAll($sql);
}
/**
 * 取得可用的自提点列表
...
{
    $andwhere = " and a.`supp_id` = '" . $suppliers_id . "' ";
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' . 's.shipping_desc, s.insure, s.support_cod, a.configure ' . 'FROM ' . $GLOBALS['hhs']->table('shipping') . ' AS s, ' . $GLOBALS['hhs']->table('shipping_area') . ' AS a, ' . $GLOBALS['hhs']->table('area_region') . ' AS r ' . "WHERE s.shipping_id = '{$shipping_id}' " . 'AND r.region_id ' . db_create_in($region_id_list) . $andwhere . ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1';
...
{
    $sql = 'SELECT pay_id, pay_name ' . 'FROM ' . $GLOBALS['hhs']->table('payment') . ' WHERE enabled = 1';
    return $GLOBALS['db']->getAll($sql);
}
/**
 * 取得支付方式信息
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['hhs']->table('payment') . " WHERE pay_id = '{$pay_id}' AND enabled = 1";
    return $GLOBALS['db']->getRow($sql);
}
/**
 * 获得订单需要支付的支付费用
...
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, is_cod' . ' FROM ' . $GLOBALS['hhs']->table('payment') . ' WHERE enabled = 1 ';
    if (!$support_cod) {
        $sql .= 'AND is_cod = 0 ';
        // 如果不支持货到付款
    }
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('pack') . " WHERE pack_id = '{$pack_id}'";
    return $GLOBALS['db']->getRow($sql);
}
/**
 * 根据订单中的商品总额来获得包装的费用
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('card') . " WHERE card_id = '{$card_id}'";
    return $GLOBALS['db']->getRow($sql);
}
/**
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0) {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['hhs']->table('order_info') . " WHERE order_id = '{$order_id}'";
    } else {
        $sql = "SELECT *, " . $total_fee . "  FROM " . $GLOBALS['hhs']->table('order_info') . " WHERE order_sn = '{$order_sn}'";
    }
...
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " . "goods_price, goods_attr, is_real, parent_id, is_gift, " . "goods_price * goods_number AS subtotal, extension_code " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " WHERE order_id = '{$order_id}'";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
...
{
    $sql = "SELECT SUM(goods_price * goods_number) " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " WHERE order_id = '{$order_id}'";
    if (!$include_gift) {
        $sql .= " AND is_gift = 0";
    }
...
{
    $sql = "SELECT SUM(g.goods_weight * o.goods_number) AS weight, " . "SUM(o.goods_price * o.goods_number) AS amount ," . "SUM(o.goods_number) AS number " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " AS o, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE o.order_id = '{$order_id}' " . "AND o.goods_id = g.goods_id";
    $row = $GLOBALS['db']->getRow($sql);
...
{
    $total = array('amount_fee' => 0.0, 'shipping_fee' => 0.0, 'bonus_fee' => 0.0);
    ...
    /* 检查系统设定的运费与满包邮额度 */
    $shopConfig_freight_free = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['hhs']->table("shop_config") . " WHERE id = 10000");
    $shopConfig_shipping_free_money = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['hhs']->table("shop_config") . " WHERE id = 10001");
    if($shopConfig_shipping_free_money > 0){
        if($total['amount_fee'] >= $shopConfig_shipping_free_money){
            $total['shipping_fee'] = 0;
        } else {
            $total['shipping_fee'] = $shopConfig_freight_free;
            $total['amount_fee'] = $total['amount_fee'] + $shopConfig_freight_free; 
        }
    } else {
        $total['shipping_fee'] = $shopConfig_freight_free;
        $total['amount_fee']  += $shopConfig_freight_free; 
    }
    /* 保存订单信息 */
...
function calc_suppliers_fee($order, $goods, $consignee, $suppliers_id)
{
    ...
    if (($order['express_id'] > 0 || $order['shipping_id'] > 0) && $total['real_goods_count'] > 0) {
        if ($order['express_id'] > 0) {
            ...
            $configure = array(array('name' => 'item_fee', 'value' => $express['shipping_fee']), array('name' => 'base_fee', 'value' => $express['shipping_fee']), array('name' => 'basic_fee', 'value' => $express['shipping_fee']), array('name' => 'step_fee', 'value' => $express['step_fee']), array('name' => 'fee_compute_mode', 'value' => $express['fee_compute_mode']), array('name' => 'free_money', 'value' => ''), array('name' => 'pay_fee', 'value' => ''));
            $shipping_info = array('shipping_code' => $express['shipping_code'], 'shipping_name' => $express['shipping_name'], 'pay_fee' => 0, 'insure' => 0, 'support_cod' => 0, 'configure' => $configure);
        } else {
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['hhs']->table('order_info'), $order, 'UPDATE', "order_id = '{$order_id}'");
}
/**
 * 得到新订单号
 * @return  string
...
{
    $sql = "SELECT suppliers_id FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' " . "AND rec_type = '{$type}'";
    return $GLOBALS['db']->getAll($sql);
}
/**
 * 取得购物车商品
...
{
    $sql = "SELECT IFNULL(s.suppliers_name,'自营店') as suppliers_name,s.supp_logo,c.suppliers_id, c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.goods_img, c.is_checked, " . "c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " . "c.goods_price * c.goods_number AS subtotal " . "FROM " . $GLOBALS['hhs']->table('cart') . " as c" . " LEFT JOIN " . $GLOBALS['hhs']->table('suppliers') . " as s on s.suppliers_id = c.suppliers_id " . " WHERE c.session_id = '" . SESS_ID . "' " . " AND is_checked = 1 AND c.rec_type = '{$type}'";
    ...
    foreach ($arr as $key => $value) {
        ...
        $subtotal = isset($data[$suppliers_id]['subtotal']) ? $data[$suppliers_id]['subtotal'] : 0.0;
...
{
    $sql = "SELECT SUM(goods_price * goods_number) " . " FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' " . "AND rec_type = '{$type}' ";
    if (!$include_gift) {
...
{
    /* 检查该商品是否已经存在在购物车中 */
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('cart') . "WHERE session_id = '" . SESS_ID . "' AND goods_id = '{$id}' " . "AND parent_id = 0 AND goods_attr = '" . get_goods_attr_info($spec) . "' " . "AND rec_type = '{$type}'";
...
function cart_weight_price($type = CART_GENERAL_GOODS, $suppliers_id = 0)
{
    ...
    if ($row) {
        ...
        foreach ($row as $val) {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['hhs']->table('package_goods') . ' AS pg, ' . $GLOBALS['hhs']->table('goods') . ' AS g ' . "WHERE g.goods_id = pg.goods_id AND g.is_shipping = 0 AND pg.package_id = '" . $val['goods_id'] . "'";
            $shipping_count = $GLOBALS['db']->getOne($sql);
            if ($shipping_count > 0) {
                // 循环计算每个超值礼包商品的重量和数量，注意一个礼包中可能包换若干个同一商品
                $sql = 'SELECT SUM(g.goods_weight * pg.goods_number) AS weight, ' . 'SUM(pg.goods_number) AS number FROM ' . $GLOBALS['hhs']->table('package_goods') . ' AS pg, ' . $GLOBALS['hhs']->table('goods') . ' AS g ' . "WHERE g.goods_id = pg.goods_id AND g.is_shipping = 0 AND pg.package_id = '" . $val['goods_id'] . "'";
    ...
    /* 获得购物车中非超值礼包商品的总重量 */
    $sql = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' . 'SUM(c.goods_price * c.goods_number) AS amount, ' . 'SUM(c.goods_number) AS number ' . 'FROM ' . $GLOBALS['hhs']->table('cart') . ' AS c ' . 'LEFT JOIN ' . $GLOBALS['hhs']->table('goods') . ' AS g ON g.goods_id = c.goods_id ' . "WHERE c.session_id = '" . SESS_ID . "' " . $andwhere . "AND rec_type = '{$type}' AND g.is_shipping = 0 AND c.extension_code != 'package_buy'";
    $row = $GLOBALS['db']->getRow($sql);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $_parent_id = $parent;
    /* 取得商品信息 */
    $sql = "SELECT g.rate_1,g.rate_2,g.rate_3,g.limit_buy_one,is_miao, g.limit_buy_bumber, g.is_fresh, g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, " . "g.market_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date, " . "g.promote_end_date, g.goods_weight, g.integral, g.extension_code,g.goods_thumb as 'goods_img',g.suppliers_id,  " . "g.goods_number, g.is_alone_sale, g.is_shipping," . "IFNULL(mp.user_price, g.shop_price * '{$_SESSION['discount']}') AS shop_price " . " FROM " . $GLOBALS['hhs']->table('goods') . " AS g " . " LEFT JOIN " . $GLOBALS['hhs']->table('member_price') . " AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '{$_SESSION['user_rank']}' " . " WHERE g.goods_id = '{$goods_id}'" . " AND g.is_delete = 0";
    ...
    if ($parent > 0) {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('cart') . " WHERE goods_id='{$parent}' AND session_id='" . SESS_ID . "' AND extension_code <> 'package_buy'";
    ...
    /* 初始化要插入购物车的基本件数据 */
    $parent = array('user_id' => $_SESSION['user_id'], 'session_id' => SESS_ID, 'goods_id' => $goods_id, 'goods_sn' => addslashes($goods['goods_sn']), 'product_id' => $product_info['product_id'], 'goods_name' => addslashes($goods['goods_name']), 'goods_img' => $goods['goods_img'], 'market_price' => $goods['market_price'], 'suppliers_id' => $goods['suppliers_id'], 'goods_attr' => addslashes($goods_attr), 'goods_attr_id' => $goods_attr_id, 'is_real' => $goods['is_real'], 'extension_code' => $goods['extension_code'], 'is_gift' => 0, 'is_shipping' => $goods['is_shipping'], 'rate_1' => $goods['rate_1'], 'rate_2' => $goods['rate_2'], 'rate_3' => $goods['rate_3'], 'rec_type' => CART_GENERAL_GOODS);
    /* 如果该配件在添加为基本件的配件时，所设置的“配件价格”比原价低，即此配件在价格上提供了优惠， */
    /* 则按照该配件的优惠价格卖，但是每一个基本件只能购买一个优惠价格的“该配件”，多买的“该配件”不享 */
    /* 受此优惠 */
    $basic_list = array();
    $sql = "SELECT parent_id, goods_price " . "FROM " . $GLOBALS['hhs']->table('group_goods') . " WHERE goods_id = '{$goods_id}'" . " AND goods_price < '{$goods_price}'" . " AND parent_id = '{$_parent_id}'" . " ORDER BY goods_price";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
    ...
    if ($basic_list) {
        $sql = "SELECT goods_id, SUM(goods_number) AS count " . "FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "'" . " AND parent_id = 0" . " AND extension_code <> 'package_buy' " . " AND goods_id " . db_create_in(array_keys($basic_list)) . " GROUP BY goods_id";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $GLOBALS['db']->fetchRow($res)) {
    ...
    /* 一个基本件对应一个该商品配件 */
    if ($basic_count_list) {
        $sql = "SELECT parent_id, SUM(goods_number) AS count " . "FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "'" . " AND goods_id = '{$goods_id}'" . " AND extension_code <> 'package_buy' " . " AND parent_id " . db_create_in(array_keys($basic_count_list)) . " GROUP BY parent_id";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $GLOBALS['db']->fetchRow($res)) {
    ...
    if ($num > 0) {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND goods_id = '{$goods_id}' " . " AND parent_id = 0 AND goods_attr = '" . get_goods_attr_info($spec) . "' " . " AND extension_code <> 'package_buy' " . " AND rec_type = 'CART_GENERAL_GOODS'";
        $row = $GLOBALS['db']->getRow($sql);
        if ($row) {
            //如果购物车已经有此物品，则更新
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage) {
                ...
                $sql = "UPDATE " . $GLOBALS['hhs']->table('cart') . " SET goods_number = '{$num}'" . " , goods_price = '{$goods_price}'" . " WHERE session_id = '" . SESS_ID . "' AND goods_id = '{$goods_id}' " . " AND parent_id = 0 AND goods_attr = '" . get_goods_attr_info($spec) . "' " . " AND extension_code <> 'package_buy' " . "AND rec_type = 'CART_GENERAL_GOODS'";
...
{
    $sql = "DELETE FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND rec_type = '{$type}'";
    $GLOBALS['db']->query($sql);
}
/**
 * 获得指定的商品属性
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr)) {
        $fmt = "%s:%s \n";
        // $fmt = "%s:%s[%s] \n";
        $sql = "SELECT a.attr_name, ga.attr_value, ga.attr_price " . "FROM " . $GLOBALS['hhs']->table('goods_attr') . " AS ga, " . $GLOBALS['hhs']->table('attribute') . " AS a " . "WHERE " . db_create_in($arr, 'ga.goods_attr_id') . " AND a.attr_id = ga.attr_id";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $GLOBALS['db']->fetchRow($res)) {
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['hhs']->table('users'), $user, 'UPDATE', "user_id = '{$user_id}'");
}
/**
 * 取得用户地址列表
 * @param   int     $user_id    用户id
...
{
    $sql = "SELECT * FROM " . $GLOBALS['hhs']->table('user_address') . " WHERE user_id = '{$user_id}'";
    return $GLOBALS['db']->getAll($sql);
}
/**
 * 取得用户地址信息
...
function user_bonus($user_id, $goods_amount = 0, $suppliers_id = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name,b.user_id,t.suppliers_id, t.type_money, t.min_goods_amount, b.bonus_id " . "FROM " . $GLOBALS['hhs']->table('bonus_type') . " AS t," . $GLOBALS['hhs']->table('user_bonus') . " AS b " . "WHERE t.type_id = b.bonus_type_id " . $andwhere . "AND t.free_all = 0 " . "AND t.use_start_date <= '{$today}' " . "AND t.use_end_date >= '{$today}' " . "AND t.min_goods_amount <= '{$goods_amount}' " . "AND b.user_id<>0 " . "AND b.user_id = '{$user_id}' " . "AND b.order_id = 0";
...
function user_bonus_ty($user_id, $goods_amount = 0)
{
    $day = getdate();
    $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    $sql = "SELECT t.type_id, t.type_name,t.suppliers_id, t.type_money, t.min_goods_amount, b.bonus_id, t.tongyong " . "FROM " . $GLOBALS['hhs']->table('bonus_type') . " AS t," . $GLOBALS['hhs']->table('user_bonus') . " AS b " . "WHERE t.type_id = b.bonus_type_id " . $andwhere . "AND t.free_all = 0 " . "AND t.use_start_date <= '{$today}' " . "AND t.use_end_date >= '{$today}' " . "AND t.min_goods_amount <= '{$goods_amount}' " . "AND b.user_id<>0 " . "AND t.tongyong= 1 " . "AND b.user_id = '{$user_id}' " . "AND b.order_id = 0";
    $list = $GLOBALS['db']->getAll($sql);
    return $list;
}
/**
...
{
    $sql = "SELECT t.*, b.* " . "FROM " . $GLOBALS['hhs']->table('bonus_type') . " AS t," . $GLOBALS['hhs']->table('user_bonus') . " AS b " . "WHERE t.type_id = b.bonus_type_id ";
    if ($bonus_id > 0) {
...
{
    $sql = "SELECT order_id FROM " . $GLOBALS['hhs']->table('user_bonus') . " WHERE bonus_id = '{$bonus_id}'";
    return $GLOBALS['db']->getOne($sql) > 0;
}
/**
 * 设置红包为已使用
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('user_bonus') . " SET order_id = '{$order_id}', used_time = '" . gmtime() . "' " . "WHERE bonus_id = '{$bonus_id}' LIMIT 1";
...
{
    $sql = "UPDATE " . $GLOBALS['hhs']->table('user_bonus') . " SET order_id = 0, used_time = 0 " . "WHERE bonus_id = '{$bonus_id}' LIMIT 1";
    return $GLOBALS['db']->query($sql);
}
/**
 * 计算积分的价值（能抵多少钱）
...
function get_cart_goods()
{
    ...
    /* 循环、统计 */
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " . " FROM " . $GLOBALS['hhs']->table('cart') . " " . " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" . " ORDER BY pid, parent_id";
    $res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (trim($row['goods_attr']) != '') {
            ...
            $sql = "SELECT attr_value FROM " . $GLOBALS['hhs']->table('goods_attr') . " WHERE goods_attr_id " . db_create_in($row['goods_attr']);
...
function get_consignee($user_id)
{
    if (isset($_SESSION['flow_consignee'])) {
        ...
        if ($user_id > 0) {
            /* 取默认地址 */
            $sql = "SELECT ua.*" . " FROM " . $GLOBALS['hhs']->table('user_address') . "AS ua, " . $GLOBALS['hhs']->table('users') . ' AS u ' . " WHERE u.user_id='{$user_id}' AND ua.address_id = u.address_id";
...
{
    if ($order_id <= 0) {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND is_real = 1 " . "AND rec_type = '{$flow_type}'";
    } else {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('order_goods') . " WHERE order_id = '{$order_id}' AND is_real = 1";
    }
    return $GLOBALS['db']->getOne($sql) > 0;
}
/**
...
{
    $sql = "SELECT shipping_id, pay_id " . " FROM " . $GLOBALS['hhs']->table('order_info') . " WHERE user_id = '{$_SESSION['user_id']}' " . " ORDER BY order_id DESC LIMIT 1";
    $row = $GLOBALS['db']->getRow($sql);
    if (empty($row)) {
        /* 如果获得是一个空数组，则返回默认值 */
...
function get_total_bonus($suppliers_id)
{
    ...
    $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    /* 按商品发的红包 */
    $sql = "SELECT SUM(c.goods_number * t.type_money)" . "FROM " . $GLOBALS['hhs']->table('cart') . " AS c, " . $GLOBALS['hhs']->table('bonus_type') . " AS t, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE c.session_id = '" . SESS_ID . "' " . "AND c.is_gift = 0 " . "AND c.goods_id = g.goods_id " . "AND g.bonus_type_id = t.type_id " . "AND t.send_type = '" . SEND_BY_GOODS . "' " . "AND t.send_start_date <= '{$today}' " . "AND t.send_end_date >= '{$today}' " . "AND c.suppliers_id = '" . $suppliers_id . "' " . "AND c.rec_type = '" . CART_GENERAL_GOODS . "'";
    $goods_total = floatval($GLOBALS['db']->getOne($sql));
    /* 取得购物车中非赠品总金额 */
    $sql = "SELECT SUM(goods_price * goods_number) " . "FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' " . " AND is_gift = 0 " . " AND suppliers_id = '" . $suppliers_id . "' " . " AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $amount = floatval($GLOBALS['db']->getOne($sql));
    /* 按订单发的红包 */
    $sql = "SELECT FLOOR('{$amount}' / min_amount) * type_money " . "FROM " . $GLOBALS['hhs']->table('bonus_type') . " WHERE send_type = '" . SEND_BY_ORDER . "' " . " AND send_start_date <= '{$today}' " . "AND send_end_date >= '{$today}' " . "AND min_amount > 0 ";
...
{
    if ($is_used) {
        $sql = 'UPDATE ' . $GLOBALS['hhs']->table('user_bonus') . ' SET ' . 'used_time = ' . gmtime() . ', ' . "order_id = '{$order_id}' " . "WHERE bonus_id = '{$bonus_id}'";
    } else {
        $sql = 'UPDATE ' . $GLOBALS['hhs']->table('user_bonus') . ' SET ' . 'used_time = 0, ' . 'order_id = 0 ' . "WHERE bonus_id = '{$bonus_id}'";
    }
    $GLOBALS['db']->query($sql);
}
/**
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    /* 更新订单商品 */
    $sql = 'UPDATE ' . $GLOBALS['hhs']->table('order_goods') . " SET order_id = '{$order_id}' " . "WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']));
    $GLOBALS['db']->query($sql);
    include_once ROOT_PATH . 'includes/lib_clips.php';
    /* 插入支付日志 */
    insert_pay_log($order_id, $order['order_amount'], PAY_ORDER);
    /* 删除原订单 */
    $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('order_info') . " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']));
    $GLOBALS['db']->query($sql);
    /* 删除原订单支付日志 */
    $sql = 'DELETE FROM ' . $GLOBALS['hhs']->table('pay_log') . " WHERE order_id " . db_create_in(array($from_order['order_id'], $to_order['order_id']));
    $GLOBALS['db']->query($sql);
...
function get_agency_by_regions($regions)
{
    ...
    $arr = array();
    $sql = "SELECT region_id, agency_id " . "FROM " . $GLOBALS['hhs']->table('region') . " WHERE region_id " . db_create_in($regions) . " AND region_id > 0 AND agency_id > 0";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    switch ($storage) {
        case 0:
            $sql = "SELECT goods_id, SUM(send_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['hhs']->table('order_goods') . " WHERE order_id = '{$order_id}' AND is_real = 1 GROUP BY goods_id, product_id";
            break;
        case 1:
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id FROM " . $GLOBALS['hhs']->table('order_goods') . " WHERE order_id = '{$order_id}' AND is_real = 1 GROUP BY goods_id, product_id";
            break;
    }
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($row['extension_code'] != "package_buy") {
            ...
            $sql = "SELECT goods_id, goods_number" . " FROM " . $GLOBALS['hhs']->table('package_goods') . " WHERE package_id = '" . $row['goods_id'] . "'";
            $res_goods = $GLOBALS['db']->query($sql);
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods)) {
                $sql = "SELECT is_real" . " FROM " . $GLOBALS['hhs']->table('goods') . " WHERE goods_id = '" . $row_goods['goods_id'] . "'";
...
function compute_discount()
{
    ...
    $sql = "SELECT *" . "FROM " . $GLOBALS['hhs']->table('favourable_activity') . " WHERE start_time <= '{$now}'" . " AND end_time >= '{$now}'" . " AND CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'" . " AND act_type " . db_create_in(array(FAT_DISCOUNT, FAT_PRICE));
    $favourable_list = $GLOBALS['db']->getAll($sql);
    if (!$favourable_list) {
        return 0;
    }
    /* 查询购物车商品 */
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " . "FROM " . $GLOBALS['hhs']->table('cart') . " AS c, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE c.goods_id = g.goods_id " . "AND c.session_id = '" . SESS_ID . "' " . "AND c.parent_id = 0 " . "AND c.is_gift = 0 " . "AND rec_type = '" . CART_GENERAL_GOODS . "'";
...
{
    $sql = "SELECT SUM(c.goods_number * IF(g.give_integral > -1, g.give_integral, c.goods_price))" . "FROM " . $GLOBALS['hhs']->table('cart') . " AS c, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE c.goods_id = g.goods_id " . "AND c.session_id = '" . SESS_ID . "' " . "AND c.goods_id > 0 " . "AND c.parent_id = 0 " . "AND c.rec_type = 0 " . "AND c.is_gift = 0";
...
function integral_to_give($order)
{
    ...
    if ($order['extension_code'] == 'group_buy') {
        ...
        $sql = "SELECT SUM(og.goods_number * IF(g.give_integral > -1, g.give_integral, og.goods_price)) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " AS og, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE og.goods_id = g.goods_id " . "AND og.order_id = '{$order['order_id']}' " . "AND og.goods_id > 0 " . "AND og.parent_id = 0 " . "AND og.is_gift = 0 AND og.extension_code != 'package_buy'";
        return $GLOBALS['db']->getRow($sql);
    }
}
/**
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list) {
        /* 用户信息 */
        $sql = "SELECT u.openid, u.user_id, u.user_name, u.email, o.team_first " . "FROM " . $GLOBALS['hhs']->table('order_info') . " AS o, " . $GLOBALS['hhs']->table('users') . " AS u " . "WHERE o.order_id = '{$order_id}' " . "AND o.user_id = u.user_id ";
        $user = $GLOBALS['db']->getRow($sql);
        /* 统计 */
        $count = 0;
        $money = '';
        foreach ($bonus_list as $bonus) {
            ...
            /* 修改用户优惠劵 */
            if ($bonus['is_share'] == 1) {
                $sql = "INSERT INTO " . $GLOBALS['hhs']->table('send_bonus_type') . " (user_id, type_id,add_time,send_order_id,send_number) " . "VALUES( '{$user['user_id']}','{$bonus['type_id']}','" . gmtime() . "','{$order_id}','{$bonus['share_number']}')";
                $GLOBALS['db']->query($sql);
                $send_id = $GLOBALS['db']->insert_id();
                //好友券自己不能用
                for ($i = 0; $i < $bonus['share_number']; $i++) {
                    $sql = "INSERT INTO " . $GLOBALS['hhs']->table('user_bonus') . " (bonus_type_id, send_user_id,send_order_id,send_id,suppliers_id) " . "VALUES('{$bonus['type_id']}', '{$user['user_id']}','{$order_id}','{$send_id}','{$suppliers_id}')";
                    $GLOBALS['db']->query($sql);
                    /*
                ...
                $sql = "INSERT INTO " . $GLOBALS['hhs']->table('user_bonus') . " (bonus_type_id, user_id,send_order_id,suppliers_id) " . "VALUES('{$bonus['type_id']}', '{$user['user_id']}','{$order_id}','{$suppliers_id}')";
                for ($i = 0; $i < $bonus['number']; $i++) {
                    if (!$GLOBALS['db']->query($sql)) {
                        return $GLOBALS['db']->errorMsg();
                    }
...
function return_order_bonus($order_id)
{
    ...
    if ($bonus_list) {
        ...
        $user_id = $order['user_id'];
        foreach ($bonus_list as $bonus) {
            $sql = "DELETE FROM " . $GLOBALS['hhs']->table('user_bonus') . " WHERE bonus_type_id = '{$bonus['type_id']}' " . "AND user_id = '{$user_id}' " . "AND order_id = '0' LIMIT " . $bonus['number'];
            $GLOBALS['db']->query($sql);
        }
    }
}
...
function order_bonus($order_id, $suppliers_id)
{
    ...
    $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    //发放的普通优惠券
    $sql = "SELECT b.free_all, b.type_id, b.type_money,b.is_share, SUM(o.goods_number) AS number,b.number as share_number,use_start_date,use_end_date " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " AS o, " . $GLOBALS['hhs']->table('goods') . " AS g, " . $GLOBALS['hhs']->table('bonus_type') . " AS b " . " WHERE o.order_id = '{$order_id}' " . " AND o.is_gift = 0 " . " AND o.goods_id = g.goods_id " . " AND g.bonus_type_id = b.type_id  " . " AND b.suppliers_id = '{$suppliers_id}' " . " AND b.send_type = '" . SEND_BY_GOODS . "' " . " AND b.send_start_date <= '{$today}' " . " AND b.send_end_date >= '{$today}' " . " GROUP BY b.type_id ";
    $list = $GLOBALS['db']->getAll($sql);
    //分享的优惠券
    $sql = "SELECT b.free_all, b.type_id, b.type_money,b.is_share, 1 as number,b.number as share_number,use_start_date,use_end_date  " . "FROM " . $GLOBALS['hhs']->table('order_goods') . " AS o, " . $GLOBALS['hhs']->table('goods') . " AS g, " . $GLOBALS['hhs']->table('bonus_type') . " AS b " . " WHERE o.order_id = '{$order_id}' " . " AND o.is_gift = 0 " . " AND o.goods_id = g.goods_id " . " AND b.suppliers_id = '{$suppliers_id}' " . " AND  g.share_bonus_type = b.type_id  " . " AND b.send_type = '" . SEND_BY_GOODS . "' " . " AND b.send_start_date <= '{$today}' " . " AND b.send_end_date >= '{$today}' " . " GROUP BY b.type_id ";
    $list2 = $GLOBALS['db']->getAll($sql);
    /* 查询定单中非赠品总金额 */
    $amount = order_amount($order_id, false);
    /* 查询订单日期 */
    $sql = "SELECT add_time " . " FROM " . $GLOBALS['hhs']->table('order_info') . " WHERE order_id = '{$order_id}' LIMIT 1";
    $order_time = $GLOBALS['db']->getOne($sql);
    /* 查询按订单发的优惠劵 */
    $sql = "SELECT free_all, type_id, type_money,is_share ,IFNULL(FLOOR('{$amount}' / min_amount), 1) AS number , number as share_number,use_start_date,use_end_date " . " FROM " . $GLOBALS['hhs']->table('bonus_type') . "WHERE send_type = '" . SEND_BY_ORDER . "' " . " AND suppliers_id = '{$suppliers_id}' " . "AND send_start_date <= '{$order_time}' " . "AND min_amount <= '{$amount}' " . "AND send_end_date >= '{$order_time}' order by type_money DESC LIMIT 1";
    //echo $sql;exit;
...
function compute_discount_amount($suppliers_id)
{
    ...
    /* 查询购物车商品 */
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " . "FROM " . $GLOBALS['hhs']->table('cart') . " AS c, " . $GLOBALS['hhs']->table('goods') . " AS g " . "WHERE c.goods_id = g.goods_id " . "AND c.session_id = '" . SESS_ID . "' " . "AND c.parent_id = 0 " . "AND c.is_gift = 0 " . "AND c.suppliers_id = '" . $suppliers_id . "' " . "AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $goods_list = $GLOBALS['db']->getAll($sql);
    if (!$goods_list) {
        return 0;
    }
...
function add_package_to_cart($package_id, $num = 1)
{
    ...
    if ($num > 0) {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " . $GLOBALS['hhs']->table('cart') . " WHERE session_id = '" . SESS_ID . "' AND goods_id = '" . $package_id . "' " . " AND parent_id = 0 AND extension_code = 'package_buy' " . " AND rec_type = '" . CART_GENERAL_GOODS . "'";
        $row = $GLOBALS['db']->getRow($sql);
        if ($row) {
            //如果购物车已经有此物品，则更新
            $num += $row['goods_number'];
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num > 0) {
                $sql = "UPDATE " . $GLOBALS['hhs']->table('cart') . " SET goods_number = '" . $num . "'" . " WHERE session_id = '" . SESS_ID . "' AND goods_id = '{$package_id}' " . " AND parent_id = 0 AND extension_code = 'package_buy' " . " AND rec_type = '" . CART_GENERAL_GOODS . "'";
                $GLOBALS['db']->query($sql);
            } else {
                $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['shortage'], $num), ERR_OUT_OF_STOCK);
                return false;
