...
function order_fee($order, $goods, $consignee)
{
    ...
    if($shopConfig_shipping_free_money > 0){
        if($total['amount'] < $shopConfig_shipping_free_money){
            /* 如果不满足包邮条件则加运费 */
            $total['amount'] += $shopConfig_freight_free;
            $total['shipping_fee_formated'] = $shopConfig_freight_free;
        } else {
            $total['shipping_fee_formated'] = 0;
        }
    } else {
        $total['amount'] += $shopConfig_freight_free;
        $total['shipping_fee_formated'] = $shopConfig_freight_free;
