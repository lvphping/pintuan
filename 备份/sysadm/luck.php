...
if($_REQUEST['act'] == 'luck_start'){
	...
	if(isset($_REQUEST['checkboxes']))
	{
		...
		$doing_team_sign_id = $GLOBALS['db']->getAll($doing_team_sign_sql);
		
		
		if($doing_team_sign_id)
		{
			...
			foreach($doing_team_sign_id as $key => $value)
			{
			...
			$links[] = array('text' =>'抽奖活动详情', 'href' => 'luckdraw.php?act=view&snatch_id=' . $luck_id);
