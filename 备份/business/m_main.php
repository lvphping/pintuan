...
if($action =='default')
{
    ...
    $yihe = $db->getOne("select COUNT(order_id) as count from ".$hhs->table('order_info')." where shipping_id='30' and op_uid!='' and suppliers_id='{$_SESSION['suppliers_id']}'");
    $weihe = $db->getOne("select COUNT(order_id) as count from ".$hhs->table('order_info')." where shipping_id='30' and op_uid='' and suppliers_id='{$_SESSION['suppliers_id']}'");
