...
if ($_REQUEST['from'] && isset($_REQUEST['isappinstalled']) && $u_id != $_SESSION['user_id'] && $u_id) {
    $sql = "select *,count(user_id) as count from " . $hhs->table('share_info') . " where user_id = {$u_id}";
    //查询是否有分享记录
    $rs = $db->getRow($sql);
    if ($rs['count'] > 0) {
        $sql = "select allow_sharej,share_j from " . $hhs->table('goods') . " where goods_id = {$g_id}";
        //查询该商品是否有分享得积分
        $row = $db->getRow($sql);
        if ($rs['child_id'] != '') {
...
$link = "http://" . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '?id=' . $goods_id . '&uid=' . $uid;
//"/goods.php";
...
if ($goods['is_nearby']) {
    ...
    /**/
    $sql = "select oi.team_sign,oi.pay_time,g.goods_name,g.team_price,u.user_name,u.uname,u.headimgurl, (oi.team_num-oi.teammen_num ) as progress from " . $hhs->table("order_info") . " as oi left join " . $hhs->table("goods") . " as g on oi.extension_id=g.goods_id left join " . $hhs->table('users') . " as u on oi.user_id=u.user_id where oi.team_first=1 and oi.team_status=1 and oi.pay_status = 2 and oi.extension_code='team_goods' and oi.extension_id='{$goods_id}' ";
    // echo $sql;
...
if ($goods['is_luck'] > 0) {
    ...
    $smarty->assign('left_num', $goods['team_num'] - $left_num);
    //剩余的人
...
{
    $sql = "SELECT SUM(attr_price) FROM " . $GLOBALS['hhs']->table('goods_attr') . " WHERE goods_id='{$goods_id}' AND " . db_create_in($attr, 'goods_attr_id');
    return $GLOBALS['db']->getOne($sql);
}
function get_buy_sum($goods_id)
{
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' . 'FROM ' . $GLOBALS['hhs']->table('order_info') . ' AS o, ' . $GLOBALS['hhs']->table('order_goods') . ' AS g ' . "WHERE o.order_id = g.order_id " . "AND o.order_status  in (0,1,5)  " . " AND o.shipping_status in (0,1,2) " . " AND o.pay_status in (1,2) " . " AND g.goods_id = " . $goods_id;
    return $GLOBALS['db']->getOne($sql);
}
